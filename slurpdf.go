...
// are expected to two contains strings, (a timestamp string and a
// symbol string, typically);
...
func (df *SlurpDataFrame) ReadGzipped(path string) (err error) {
	...
	if df.Missing2strings {
		...
		skip = 2
		kind = append(kind, TIMESTAMP)
		kind = append(kind, SYMBOL)

		// we have tm,sym up front
		if len(firstRow) < 3 {
			panic(fmt.Sprintf("len(firstRow) = %v BUT d.Missing2strings if false, so what gives? firstRow='%#v'", len(firstRow), firstRow))
		}
		vv("missing 2 strings false. Allocating df.Tm")
		df.Tm = make([]time.Time, nrow)
		df.Sym = firstRow[1]
		firstRow = firstRow[2:] // we know the first two tm,sym are strings, do not confuse them with factors.
	}

	for j, fld := range firstRow {
		_, err1 := strconv.ParseFloat(fld, 64)
		if err1 == nil {
			numeric[j] = true
			//vv("possibly numeric field: '%v' with example '%v'", loader.Header[j], fld)
			kind = append(kind, NUMERIC)
			df.numericNames = append(df.numericNames, loader.Header[j+skip])
			...
			df.HasFactors = true
			df.factorNames = append(df.factorNames, loader.Header[j+skip])
	...
	vv("len(numeric) = %v; len(factors) = %v", len(numeric), len(factors))

	df.Kindvec = kind
	df.Ncol = len(r[0])

	df.Nrow = nrow
	df.NumericMat = NewMatrixFloat64(nrow, len(numeric))
	df.NumericMat.Colnames = df.numericNames

	df.FactorMat = NewMatrixInt(nrow, len(factors))
	...
	for i := range r {
		...
		for j, fld := range r[i] {
			...
			if !df.Missing2strings && j < 2 {
				switch j {
				case 0:
					// tm
					df.Tm[i], err = time.Parse(time.RFC3339Nano, fld)
					panicOn(err)
				case 1:
					// symbol
				}
...
func (df *SlurpDataFrame) Slurp(path string) (err error) {
	...
	if df.Missing2strings {
		...
		//vv("df.MatrixColnames = '%#v'", df.MatrixColnames)
	}
...
// RowSlice can use nSpan to request just a subset of columns.
...
// The Y and X conventions are that of regression in statistics. Y is the
// target (or targets) to be predicted, X is the set of independent predictors.
//
