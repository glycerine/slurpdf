...
type MatrixFloat64 struct {
	...
	// avoid allocation on Row() for column-major
	// Matrix by re-using this internal buffer to return rows.
	rowbuffer []float64
}
...
// FeatMeta gives Feature Metadata for each column, especially details
// about factors and their levels, cuts, and IsOrdinal
...
func (f *FeatMeta) String() (r string) {
	return fmt.Sprintf("FeatMeta{len(Levels):%v; len(FactorMap)=%v; len(InvFactorMap)=%v;  %#v", len(f.Levels), len(f.FactorMap), len(f.InvFactorMap), f)
}

// NewFeatMeta creates a new feature metadata struct, used to
// describe a feature (factor, covariate, independent variable)
// that typically resides in each of the columns of our Matrix.
// The New function is necessary since the internal maps must be allocated.
func NewFeatMeta() *FeatMeta {
	return &FeatMeta{
		FactorMap:    make(map[string]int),
		InvFactorMap: make(map[int]string),
	}
}
...
func (m *MatrixFloat64) Transpose() {
	if m.IsSliceOfColVec {
		panic("m.IsSliceOfColVec is true, so MatrixFloat64.Transpose() is not allowed")
	}
...
func (m *MatrixFloat64) initCmeta() {
...
func (m *MatrixFloat64) Row(i int) (res []float64) {
	...
	if m.IsColMajor {
		// do less allocation if possible, by re-using m.rowbuffer
		m.rowbuffer = m.rowbuffer[:cap(m.rowbuffer)]
		if len(m.rowbuffer) < m.Ncol {
			// make sure we have enough space.
			m.rowbuffer = make([]float64, m.Ncol)
		}
		return m.RowInto(i, m.rowbuffer)
	}
...
// RowInto allows less allocation, compared to Row(),
// by having the caller provide a fillme slice as a
// working buffer possible fill in
// and return a slice of in the returned res slice.
//
// This is really only useful when m.IsColMajor is true.
//
// For a column-major Matrix, we will return the answer in res,
// using fillme to fill in the answer. For row-major, res
// will point to .Dat rather than fillme, to avoid allocation.
//
// The caller should just use res to be agnostic to
// the Matrix internal format. The provided fillme must be at
// least m.Ncol long, else we will panic.
//
// Rather than require the caller to provide fillme,
// we will just keep it inside Matrix in rowbuffer now.
// Row() will now automatically use RowInto with rowbuffer if
// m is column major.
func (m *MatrixFloat64) RowInto(i int, fillme []float64) (res []float64) {
	...
	if len(fillme) < m.Ncol {
		panic(fmt.Sprintf("RowInto error: fillme len %v must be >= m.Ncol(%v)", len(fillme), m.Ncol))
	}

	if m.IsSliceOfColVec {
		for j := 0; j < m.Ncol; j++ {
			fillme[j] = m.ColVec[j].Dat[i]
		}
		return fillme[:m.Ncol]
	}

	if m.IsColMajor {
		if m.Ncol == 1 {
			return m.Dat[i : i+1] // slice is only 1 element long
		}
		//panic("Row called on multi-column Column-major Matrix. Updates/changes to the resulting slice will not be reflected in the Matrix, since the result is a separate copy, not refering to the original. Be sure this is what you expect! (or just ReformatToRowMajor() this Matrix first, if you needs updates to be visible..")
		...
		// row-major can just return the data directly.
		return m.Dat[i*m.Ncol : (i+1)*m.Ncol]
	}
}
...
func (m *MatrixFloat64) RowChunk(beg, endx int) (r *MatrixFloat64) {
	...
	if m.IsColMajor || m.IsSliceOfColVec {
		...
		r = &MatrixFloat64{Dat: m.Dat[beg*m.Ncol : endx*m.Ncol], Ncol: m.Ncol, Nrow: endx - beg, Colnames: m.Colnames}
		if len(m.Rownames) == m.Nrow {
			r.Rownames = m.Rownames[beg:endx]
		}
		return
...
// NewMatrix allocates room for nrow * ncol elements. It
// defaults to row-major
func NewMatrixFloat64(nrow, ncol int) (m *MatrixFloat64) {
	m = &MatrixFloat64{
		Nrow: nrow,
		Ncol: ncol,
		Dat:  make([]float64, nrow*ncol),
	}
	m.initCmeta()
	return
}
...
func NewMatrixColMajorFloat64(nrow, ncol int) (m *MatrixFloat64) {
	m = NewMatrixFloat64(nrow, ncol)
	m.IsColMajor = true
	return
}

// NewMatrixColVec allocates room for nrow * ncol elements
...
func NewMatrixColVecFloat64(nrow, ncol int) (m *MatrixFloat64) {
	m = &MatrixFloat64{
		...
		ColVec:          make([]*ColVecFloat64, ncol),
...
func (m *MatrixFloat64) String() (r string) {
	...
	//r = fmt.Sprintf("MatrixFloat64(Nrow=%v x Ncol=%v ; len(Dat)=%v) = [\n", v, m.Nrow, m.Ncol, len(m.Dat))
	r = fmt.Sprintf("MatrixFloat64(Nrow=%v x Ncol=%v) = [\n", m.Nrow, m.Ncol)
...
func (m *MatrixFloat64) Clone() (clone *MatrixFloat64) {
	clone = &MatrixFloat64{
		...
		ColVec: make([]*ColVecFloat64, len(m.ColVec)),
	...
	if m.IsSliceOfColVec {
		...
		nrow := m.Nrow
		clone.Dat = make([]float64, nrow*clone.Ncol)
		...
		clone.Dat = make([]float64, len(m.Dat))
...
func (m *MatrixFloat64) AddRow(rowlabel string) (i int) {
	...
	if m.IsSliceOfColVec {
		for j := range m.ColVec {
			m.ColVec[j].Dat = append(m.ColVec[j].Dat, 0)
		}
		i = m.Nrow
		m.Nrow++
		...
		i = m.Nrow
		m.Nrow++
		m.Dat = make([]float64, m.Nrow*m.Ncol)
		for j := 0; j < m.Ncol; j++ {
		...
		m.Dat = append(m.Dat, make([]float64, m.Ncol)...)
		i = m.Nrow
		m.Nrow++
	}
	if rowlabel == "" {
...
func (m *MatrixFloat64) ReformatToSliceOfColVec() {
	...
	m.ColVec = make([]*ColVecFloat64, m.Ncol)
	for j := range m.ColVec {
		cv := &ColVecFloat64{
...
func (m *MatrixFloat64) SumAll() (tot float64) {
	...
	if m.IsSliceOfColVec {
		for _, colj := range m.ColVec {
			for _, v := range colj.Dat {
				tot += v
			}
		}
	} else if m.IsColMajor {
		for j := 0; j < m.Ncol; j++ {
			for i := 0; i < m.Nrow; i++ {
				tot += m.Dat[j*m.Nrow+i]
			}
		}
	} else {
		for i := 0; i < m.Nrow; i++ {
			for j := 0; j < m.Ncol; j++ {
				tot += m.Dat[i*m.Ncol+j]
			}
		}
	}
	return
...
// RowIter refers to a row-slice of a Matrix; see
...
// this aliasing. If you change m2 after a Cbind, then those changes to m2 may (or may not)
// show up also in m. For safety, discard references to m2 after Cbind()-ing it to m.
// Instead, if need be, read/write through m to the appended columns.
func (m *MatrixFloat64) Cbind(m2 *MatrixFloat64) {
	...
	if m.IsSliceOfColVec {
		...
		if m2.IsSliceOfColVec {
			...
			for j := 0; j < m2.Ncol; j++ {
				m.ColVec = append(m.ColVec, &ColVecFloat64{Dat: m2.Dat[j*m2.Nrow : (j+1)*m2.Nrow]})
			}
		}
	} else {
		...
		if !m2.IsSliceOfColVec {
			...
			m.IsSliceOfColVec = true
			m.ColVec = make([]*ColVecFloat64, nc0)
			for j := range m.ColVec {
				cv := &ColVecFloat64{}
				m.ColVec[j] = cv
				cv.Dat = m.Dat[j*m.Nrow : (j+1)*m.Nrow]
...
// RowColIter is a Matrix iterator that returns, upon Fetch, chunks of rows in
...
// If len(wcol) == 0, we will give all the columns.
