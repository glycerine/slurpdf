...
// VolVecInt is a column vector; a Matrix has an
...
func (m *MatrixInt) Transpose() {
	if m.IsSliceOfColVec {
		panic("m.IsSliceOfColVec is true, so MatrixInt.Transpose() is not allowed")
	}
...
func (m *MatrixInt) RowChunk(beg, endx int) (r *MatrixInt) {
	...
	if m.IsColMajor || m.IsSliceOfColVec {
		...
		r = &MatrixInt{Dat: m.Dat[beg*m.Ncol : endx*m.Ncol], Ncol: m.Ncol, Nrow: endx - beg, Colnames: m.Colnames}
		if len(m.Rownames) == m.Nrow {
			r.Rownames = m.Rownames[beg:endx]
		}
		return
...
func (m *MatrixInt) String() (r string) {
	var v int
	//r = fmt.Sprintf("Matrix[%T](Nrow=%v x Ncol=%v ; len(Dat)=%v) = [\n", v, m.Nrow, m.Ncol, len(m.Dat))
	r = fmt.Sprintf("Matrix[%T](Nrow=%v x Ncol=%v) = [\n", v, m.Nrow, m.Ncol)
...
func (m *MatrixInt) Clone() (clone *MatrixInt) {
	clone = &MatrixInt{
		...
		ColVec: make([]*ColVecInt, len(m.ColVec)),
	...
	if m.IsSliceOfColVec {
		...
		nrow := m.Nrow
		clone.Dat = make([]int, nrow*clone.Ncol)
...
func (m *MatrixInt) AddRow(rowlabel string) (i int) {
	...
	if m.IsSliceOfColVec {
		...
		m.Dat = append(m.Dat, make([]int, m.Ncol)...)
		i = m.Nrow
		m.Nrow++
	}
	if rowlabel == "" {
...
func (m *MatrixInt) ReformatToSliceOfColVec() {
	...
	m.ColVec = make([]*ColVecInt, m.Ncol)
	for j := range m.ColVec {
		cv := &ColVecInt{
...
func (m *MatrixInt) Cbind(m2 *MatrixInt) {
	...
	if m.IsSliceOfColVec {
		...
		if m2.IsSliceOfColVec {
			...
			for j := 0; j < m2.Ncol; j++ {
				m.ColVec = append(m.ColVec, &ColVecInt{Dat: m2.Dat[j*m2.Nrow : (j+1)*m2.Nrow]})
			}
		}
	} else {
