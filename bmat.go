...
func (m *BoolMatrix) Clone() (clone *BoolMatrix) {
	clone = &BoolMatrix{
		...
		ColVec: make([]*ColVecBool, len(m.ColVec)),
...
func (m *BoolMatrix) ReformatToSliceOfColVec() {
	...
	m.ColVec = make([]*ColVecBool, m.Ncol)
	for j := range m.ColVec {
		cv := &ColVecBool{
...
func (m *BoolMatrix) Cbind(m2 *BoolMatrix) {
	...
	if m.IsSliceOfColVec {
		...
		if m2.IsSliceOfColVec {
			...
			for j := 0; j < m2.Ncol; j++ {
				m.ColVec = append(m.ColVec, &ColVecBool{Dat: m2.Dat[j*m2.Nrow : (j+1)*m2.Nrow]})
			}
		}
	} else {
